...
def process_weather_data(raw_weather_data):
	...
	filtered_hourly_weather = list(filter(filterFunction, raw_weather_data))
...
def save_notable_variables(filtered_hourly_weather):
	...
	for h in filtered_hourly_weather:
		...
		if saved.get("windchill") is None or (h["windchill"] is not None and h["windchill"] < saved["windchill"]):
...
def determine_advisory(processed_weather_data):
	...
	elif windy_boolean and not fave_direction_boolean and twenty_low_temp_boolean:
